	...
	<div class="d-flex justify-content-center">
		<div class="card mt-5 col-md-8 animated bounceInDown myForm">
			<div class="card-header">
				<h4>{{ $data->nama_sopir }} {{ $data->no_polisi}}  - {{ $data->jenis_kendaraan }} </h4>
			</div>
			<div class="card-body">
				<form action="{{ url('insert_penumpang') }}" method="POST">
					@csrf
					<input type="hidden" name="id_trayek" value="{{$data->id}}" class="form-control">
					<div id="dynamic_container">
						<div class="input-group">
							<div class="input-group-prepend">
								<span class="input-group-text br-15"><i class="far fa-user"></i></span>
							</div>
							<input type="text" name="nama" placeholder="Nama Penumpang" class="form-control"/>
						</div>
                	  ...
                	  <select name="sift" class="browser-default custom-select">
                        <option selected="">Pilih Sift </option>
                        <option value="Pagi">Sift Pagi</option>
                        <option value="Siang">Sift Siang</option>
                        <option value="Sore">Sift Sore</option>
                      </select>
					</div>
